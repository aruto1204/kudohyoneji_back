		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'お客様情報', 'my-custom-blocks' ) }>
					<TextControl
						label={ __( 'お客様名', 'my-custom-blocks' ) }
						value={ customerName }
						onChange={ ( value ) =>
							setAttributes( { customerName: value } )
						}
					/>
					<TextControl
						label={ __( '役職・会社名', 'my-custom-blocks' ) }
						value={ customerTitle }
						onChange={ ( value ) =>
							setAttributes( { customerTitle: value } )
						}
					/>
					<MediaUploadCheck>
						<MediaUpload
							onSelect={ ( media ) =>
								setAttributes( { customerImage: media.url } )
							}
							allowedTypes={ [ 'image' ] }
							value={ customerImage }
							render={ ( { open } ) => (
				...
				<PanelBody title={ __( '評価設定', 'my-custom-blocks' ) }>
					<ToggleControl
						label={ __( '評価を表示', 'my-custom-blocks' ) }
						checked={ showRating }
						onChange={ ( value ) =>
					...
					{ showRating && (
						<RangeControl
							label={ __( '評価（星の数）', 'my-custom-blocks' ) }
							value={ rating }
							onChange={ ( value ) =>
